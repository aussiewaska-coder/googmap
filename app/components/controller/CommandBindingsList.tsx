...
interface CommandBindingsListProps {
    context: Context;
...
export default function CommandBindingsList({ context, profile, onBind, onClear }: CommandBindingsListProps) {
    // Get commands for this context (buttons only, not axes)
    const commands = COMMAND_DEFINITIONS.filter(
        cmd => cmd.context === context && cmd.type === 'button'
    );

    if (commands.length === 0) {
        return (
            <div className="p-6 bg-white/5 rounded-lg border border-white/10 text-center">
                <p className="text-white/60">No button commands available for this context</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-3">
            <h3 className="text-lg font-bold text-white mb-4">Button Commands</h3>
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    {isBound && (
                                        <span className="px-2 py-0.5 bg-cyan-500/20 border border-cyan-500/30 rounded text-xs text-cyan-300 font-medium">
                                            Bound
                                        </span>
                                    )}
                                </div>
